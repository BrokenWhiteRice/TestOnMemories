...
const { URI, databaseName } = require("../v01-mongodb/_config");
...
async function read(uri, databaseName, collectionName, query) {
  ...
  try {
    ...
    console.log(
      `read: Found ${result.length} documents from ${query} - ${result}`
    );
  } catch (error) {
    console.error(error);
  } finally {
...
//update(URI, databaseName, "post", { name: "John" }, updateQuery);
...
//delete_document(URI, databaseName, "post", { name: "John" });
