  ...
  */
  async runDeleteDelete(req, resp) {
    try {
      ...
      resp
        .status(500)
        .send({ error: `Error from runDeleteDelete: ${e.message}` });
    }
  }
  ...
  async runEditIdGet(req, resp) {
    ...
    try {
      ...
      resp
        .status(500)
        .send({ error: `Error from runEditIdGet : ${e.message}` });
    }
  }
  async runEditPut(req, resp) {
    ...
    console.log(
      `runEditPut id: ${req.body.id} title: ${req.body.title} date: ${req.body.date}`
    );
    let query = { _id: id };
    let update = {
      $set: { _id: id, title: req.body.title, date: req.body.date },
    };
    try {
      let res = await util.update(
        this.uri,
        this.database,
        this.posts,
        query,
  ...
  async runDetailIdGet(req, resp) {
    try {
      ...
      resp
        .status(500)
        .send({ error: `Error from runDetailIdGet: ${e.message}` });
    }
  }
}
